  ...
  class DynamicSecretCreateCassandra
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Akeyless::DynamicSecretCreateCassandra` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Akeyless::DynamicSecretCreateCassandra`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [cassandra_creation_statements, cassandra_hosts, cassandra_password, cassandra_port, cassandra_username, delete_protection, description, json, name, password_length, producer_encryption_key_name, ssl, ssl_certificate, tags, target_name, token, uid_token, user_ttl].hash
    end
