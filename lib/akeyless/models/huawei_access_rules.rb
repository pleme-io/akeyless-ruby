  ...
  class HuaweiAccessRules
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Akeyless::HuaweiAccessRules` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Akeyless::HuaweiAccessRules`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'auth_endpoint')
        self.auth_endpoint = attributes[:'auth_endpoint']
      end

      if attributes.key?(:'domain_id')
        if (value = attributes[:'domain_id']).is_a?(Array)
      ...
      if attributes.key?(:'tenant_id')
        if (value = attributes[:'tenant_id']).is_a?(Array)
          self.tenant_id = value
        end
      end

      if attributes.key?(:'tenant_name')
        if (value = attributes[:'tenant_name']).is_a?(Array)
    ...
    def hash
      [auth_endpoint, domain_id, domain_name, tenant_id, tenant_name, user_id, user_name].hash
    end
