...
module Akeyless
  class EmailPassAccessRules
    ...
    attr_accessor :enc_email_with_shared_key
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Akeyless::EmailPassAccessRules` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Akeyless::EmailPassAccessRules`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'enc_email_with_shared_key')
        self.enc_email_with_shared_key = attributes[:'enc_email_with_shared_key']
      end
    ...
    def hash
      [alg, email, enc_email_with_shared_key, hash_pass, last_reset_password, mfa_type].hash
    end
