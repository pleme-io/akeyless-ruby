  ...
  class EventForwarderCreateServiceNow
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Akeyless::EventForwarderCreateServiceNow` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Akeyless::EventForwarderCreateServiceNow`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [admin_name, admin_pwd, app_private_key_base64, auth_methods_event_source_locations, auth_type, client_id, client_secret, description, event_types, every, gateways_event_source_locations, host, items_event_source_locations, json, key, name, runner_type, targets_event_source_locations, token, uid_token, user_email].hash
    end
