...
module Akeyless
  ...
  class UpdateDBTarget
    ...
    attr_accessor :oracle_wallet_login_type

    # Oracle wallet p12 file data in base64
    attr_accessor :oracle_wallet_p12_file_data

    # Oracle wallet sso file data in base64
    attr_accessor :oracle_wallet_sso_file_data
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Akeyless::UpdateDBTarget` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Akeyless::UpdateDBTarget`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'oracle_wallet_login_type')
        self.oracle_wallet_login_type = attributes[:'oracle_wallet_login_type']
      end

      if attributes.key?(:'oracle_wallet_p12_file_data')
        self.oracle_wallet_p12_file_data = attributes[:'oracle_wallet_p12_file_data']
      end

      if attributes.key?(:'oracle_wallet_sso_file_data')
        self.oracle_wallet_sso_file_data = attributes[:'oracle_wallet_sso_file_data']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          oracle_wallet_login_type == o.oracle_wallet_login_type &&
          oracle_wallet_p12_file_data == o.oracle_wallet_p12_file_data &&
          oracle_wallet_sso_file_data == o.oracle_wallet_sso_file_data &&
          port == o.port &&
          pwd == o.pwd &&
    ...
    def hash
      [db_defined_connection_type, azure_client_id, azure_client_secret, azure_tenant_id, cloud_service_provider, cluster_mode, comment, connection_type, db_name, db_server_certificates, db_server_name, db_type, description, host, json, keep_prev_version, key, max_versions, mongodb_atlas, mongodb_atlas_api_private_key, mongodb_atlas_api_public_key, mongodb_atlas_project_id, mongodb_default_auth_db, mongodb_uri_options, name, new_name, oracle_service_name, oracle_wallet_login_type, oracle_wallet_p12_file_data, oracle_wallet_sso_file_data, port, pwd, snowflake_account, snowflake_api_private_key, snowflake_api_private_key_password, ssl, ssl_certificate, token, uid_token, update_version, user_name].hash
    end
