...
module Akeyless
  ...
  class RotateKeyOutput
    attr_accessor :classic_key_gw_url

    attr_accessor :item_type

    attr_accessor :new_item_version
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Akeyless::RotateKeyOutput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Akeyless::RotateKeyOutput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'classic_key_gw_url')
        self.classic_key_gw_url = attributes[:'classic_key_gw_url']
      end
      ...
      if attributes.key?(:'new_item_version')
        self.new_item_version = attributes[:'new_item_version']
      end
