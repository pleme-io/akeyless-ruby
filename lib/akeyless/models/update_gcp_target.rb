  ...
  class UpdateGcpTarget
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Akeyless::UpdateGcpTarget` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Akeyless::UpdateGcpTarget`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @gcp_sa_email.nil?
        invalid_properties.push('invalid value for "gcp_sa_email", gcp_sa_email cannot be nil.')
      end
    ...
    def valid?
      return false if @gcp_sa_email.nil?
    ...
    def hash
      [comment, gcp_key, gcp_sa_email, json, keep_prev_version, key, name, new_name, token, uid_token, update_version, use_gw_cloud_identity].hash
    end
